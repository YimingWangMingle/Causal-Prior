...
def train(_run):
    ...
    wandb.init(project='amorpheus', save_code=True, entity="mingatum",
               name=args.label + '_' + str(args.seed) + '_' + str(exp_name))
    # wandb.init(project='amorpheus', save_code=True, entity="lance_ming",
    #            name=args.label + '_' + str(args.seed) + '_' + str(exp_name))
        ...
        if collect_done:
            ...
            if this_training_timesteps != 0:
                ...
                for i in range(num_envs_train):
                    wandb.log({
                                "episode_reward_{}".format(envs_train_names[i]): episode_reward_list[i],
                                "episode_len_{}".format(envs_train_names[i]): episode_timesteps_list[i],
                                "total_timesteps_{}".format(envs_train_names[i]):total_timesteps,
                               })
                    writer.add_scalar(
